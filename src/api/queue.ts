...
app.post('/:id/resolve', asJson<{ Bindings: Env }>(async (c) => {
  ...
  if (body.decision === 'dismiss') {
    await setQueueStatus(c.env, id, 'dismissed');
  ...
  if (item.kind === 'meeting_classification') {
    ...
    if (cls === 'group') {
      await setQueueStatus(c.env, id, 'resolved');
    ...
    await markResolvedWithIds(c.env, id, result.personId, result.meetingId);
...
// Stamp the queue payload with the meeting/person ids the resolve produced
// and flip status to 'resolved'.
async function markResolvedWithIds(
  env: Env,
  queueId: string,
  personId: string,
  meetingId: string,
